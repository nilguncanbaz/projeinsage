...
if(isset($_POST["kaydet"])){
	...
	mkdir("../ebook/icerikYukleme/dosyalar/".$isbn); 

	if($query->rowCount()){


		mkdir("../ebook/icerikYukleme/dosyalar/".$isbn."/pages"); 
	}
	...
	if($query1->rowCount()){
		mkdir("../ebook/icerikYukleme/dosyalar/".$isbn."/pdfs");
	...
	if($query2->rowCount()){
		...
		mkdir("../ebook/icerikYukleme/dosyalar/".$isbn."/pagesText");
		
	}


	for($var=1; $var<=$sayfasayisi; $var++){

		$ekle=$db->query("INSERT INTO ebook_sayfa_resim VALUES (null,'$isbn',$var,'icerikYukleme/dosyalar/bosSayfa.jpg')", PDO::FETCH_ASSOC);
	}
	for($var2=1; $var2 <=$sayfasayisi ; $var2++) { 
		...
		$dosya = fopen("../ebook/icerikYukleme/dosyalar/".$isbn."/pagesText/".$var2.".txt", 'w') or die("Dosya açılamıyor");
